...
function DeusExMachinaPage({ collection }) { const [lens, setLens] = useState({ x: 50, y: 50, active: false }); const moveLens = event => { const rect = event.currentTarget.getBoundingClientRect(); setLens({ x: ((event.clientX - rect.left) / rect.width) * 100, y: ((event.clientY - rect.top) / rect.height) * 100, active: true }) }; return <main className="detail-page artwork-page deus-page"><header><a className="brand" href="#top">J.Z</a><a className="page-back" href="#gallery/virtual-photography">← BACK TO VIRTUAL PHOTOGRAPHY</a></header><section className="gallery-head"><p>( VIRTUAL PHOTOGRAPHY / SERIES 04 )</p><h1>{collection.title}</h1><span>{collection.note}</span></section><section className="deus-exhibition" onPointerMove={moveLens} onPointerLeave={() => setLens(state => ({ ...state, active: false }))}><Waves lineColor="#c8ff3f" backgroundColor="transparent" waveSpeedX={.1} waveSpeedY={.065} waveAmpX={100} waveAmpY={90} friction={.63} tension={.025} maxCursorMove={210} xGap={18} yGap={42} /><i className="deus-exhibition__glass" aria-hidden="true" style={{ '--lens-x': `${lens.x}%`, '--lens-y': `${lens.y}%`, opacity: lens.active ? 1 : 0 }} />{collection.files.map((file, index) => <figure className="deus-exhibition__work" style={{ '--deus-index': index }} key={file}><span>{String(index + 1).padStart(2, '0')}</span><img loading={index === 0 ? 'eager' : 'lazy'} src={`/assets/virtual/${file}`} alt={`${collection.title} 组图 ${index + 1}`} /><figcaption>{index === 0 ? 'THE MACHINE MAKES A GOD' : 'WHAT DOES IT OBEY?'}</figcaption></figure>)}</section><section className="artwork-text"><p>DEUS EX MACHINA / 04</p><h1>机械降神</h1><div><b>WORK INTRODUCTION</b><p>在科技之下，机械制造的神明从程序、材质与光影中降临。</p></div><div><b>SPIRIT / READING</b><p>它拥有神性的外观，却又受制于算法、数据与人的欲望：它仍是神，还是一种等待被命名的新存在？</p></div></section></main> }
