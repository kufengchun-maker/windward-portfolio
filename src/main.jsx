...
function ErosionArtwork({ source }) { const [active, setActive] = useState(false); const [ripples, setRipples] = useState([]); const last = useRef({ x: -999, y: -999 }); const position = event => { const rect = event.currentTarget.getBoundingClientRect(); const x = event.clientX - rect.left; const y = event.clientY - rect.top; event.currentTarget.style.setProperty('--reveal-x', `${x}px`); event.currentTarget.style.setProperty('--reveal-y', `${y}px`); return { x, y } }; const disturb = event => { const { x, y } = position(event); if (Math.hypot(x - last.current.x, y - last.current.y) < 48) return; last.current = { x, y }; const id = `${Date.now()}-${x}`; setRipples(items => [...items.slice(-3), { id, x, y }]); window.setTimeout(() => setRipples(items => items.filter(item => item.id !== id)), 1200) }; const enter = event => { position(event); setActive(true); disturb(event) }; return <div className={`erosion-stage__art erosion-artwork${active ? ' is-active' : ''}`} onPointerEnter={enter} onPointerMove={disturb} onPointerLeave={() => setActive(false)}><img className="erosion-artwork__base" src={source} alt="蚀 / Erosion" /><img className="erosion-artwork__color" src={source} alt="" aria-hidden="true" />{ripples.map(ripple => <i key={ripple.id} className="erosion-artwork__ripple" style={{ left: ripple.x, top: ripple.y }} />)}</div> }

function ErosionPage({ collection }) { const source = `/assets/virtual/${collection.files[0]}`; const [ripples, setRipples] = useState([]); const last = useRef({ x: -999, y: -999 }); const rippleBackground = event => { const rect = event.currentTarget.getBoundingClientRect(); const x = event.clientX - rect.left; const y = event.clientY - rect.top; if (Math.hypot(x - last.current.x, y - last.current.y) < 110) return; last.current = { x, y }; const id = `${Date.now()}-${x}`; setRipples(items => [...items.slice(-3), { id, x, y }]); window.setTimeout(() => setRipples(items => items.filter(item => item.id !== id)), 1500) }; return <main className="detail-page erosion-page"><header><a className="brand" href="#top">J.Z</a><a className="page-back" href="#gallery/virtual-photography">← BACK TO VIRTUAL PHOTOGRAPHY</a></header><section className="erosion-stage" onPointerMove={rippleBackground}><Ferrofluid className="erosion-stage__fluid" colors={['#ffffff', '#ffffff', '#ffffff']} speed={0.4} scale={1.9} turbulence={0.8} fluidity={0.09} rimWidth={0.4} sharpness={1} shimmer={2} glow={1.8} flowDirection="down" opacity={1} mouseInteraction mouseStrength={1} mouseRadius={0.3} />{ripples.map(ripple => <i key={ripple.id} className="erosion-stage__ripple" style={{ left: ripple.x, top: ripple.y }} />)}<div className="erosion-stage__info"><span>( 05 / VIRTUAL PHOTOGRAPHY )</span><h1>蚀<br /><em>EROSION.</em></h1><p>MOVE THE CURSOR<br />TO DISTURB THE SURFACE</p></div><ErosionArtwork source={source} /></section><section className="artwork-text"><p>EROSION / 05</p><h1>蚀</h1><div><b>WORK INTRODUCTION</b><p>关于人、消融与被侵蚀：无路可退时，被迫或主动加入这场灾难。</p></div><div><b>SPIRIT / READING</b><p>银色遍布世界的每个地方。灾难不再只是远处的景观，而成为每个人必须回应的邀请——你也要加入吗？</p></div></section></main> }
...
function SeriesPage({ collection }) {
  ...
  if (isFlowers) {
    ...
    const jumpToFlower = (index) => document.getElementById(`flower-frame-${index}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' })
    return <main className="detail-page artwork-page flower-exhibition"><header><a className="brand" href="#top">J.Z</a><a className="page-back" href="#gallery/virtual-photography">← BACK TO VIRTUAL PHOTOGRAPHY</a></header><section className="flower-scroll-stack">{flowerFiles.map((file, index) => <div className="flower-frame" id={`flower-frame-${index}`} key={file}><ScrollExpand src={`/assets/virtual/${file}`} alt={`花，虚拟摄影组图 ${index + 1}`} title={index === 0 ? '花' : ''} scrollHint={index === 0 ? 'SCROLL TO OPEN' : ''} startWidth={38} startHeight={54} startRadius={0} mediaZoom={1.16} scrollDistance={.78} holdDistance={.12}><div className="flower-frame__overlay"><p>FLOWERS / {String(index + 1).padStart(2, '0')} · 2025.11.13</p><button className="flower-switch flower-switch--prev" aria-label="上一张图片" disabled={index === 0} onClick={() => jumpToFlower(index - 1)}>←</button><button className="flower-switch flower-switch--next" aria-label="下一张图片" disabled={index === flowerFiles.length - 1} onClick={() => jumpToFlower(index + 1)}>→</button></div></ScrollExpand></div>)}</section><section className="artwork-text"><p>FLOWERS / 01</p><h1>花</h1><div><b>ABOUT THE SERIES</b><p>一组以红色花朵、蝴蝶与占卜意象构成的虚拟肖像。</p></div><div><b>SPIRIT / READING</b><p>柔软与锋利、被观看与自我编织，在同一张面孔上同时生长。</p></div></section></main>
  }
