...
function TravelLandscapePage({ area }) { const groups = [['01','CITY DRIFT / 城市漂流','街道、建筑、夜色与人在城市中的移动。'],['02','MOUNTAIN LINE / 山的轮廓','远景、地形与行进途中被切开的地平线。'],['03','WATER EDGE / 水的边界','海、湖、雨与潮汐留下的反光和距离。'],['04','PASSING DAYS / 经过的日常','旅行之外，仍在发生的光线、天气与片刻。']]; return <main className="detail-page travel-page"><header><a className="brand" href="#top">J.Z</a><PageBack /></header><section className="gallery-head"><p>( PHOTOGRAPHY ARCHIVE )</p><h1>{area.title}</h1><span>{area.text}</span></section><section className="travel-groups">{groups.map(([no,title,note]) => <article className="travel-group" key={no}><header><span>{no}</span><h2>{title}</h2><p>{note}</p></header><div className="travel-group__images">{Array.from({ length: 3 }, (_, index) => <figure key={index}><span>{no} / {String(index + 1).padStart(2, '0')}</span><b>[ UPLOAD TRAVEL IMAGE ]</b></figure>)}</div></article>)}</section></main> }
