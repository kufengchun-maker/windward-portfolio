...
function FlowerCarousel({ files }) { const [active, setActive] = useState(0); const move = direction => setActive(index => (index + direction + files.length) % files.length); useEffect(() => { const onKey = event => { if (event.key === 'ArrowLeft') move(-1); if (event.key === 'ArrowRight') move(1) }; window.addEventListener('keydown', onKey); return () => window.removeEventListener('keydown', onKey) }); return <section className="flower-carousel" aria-label="花系列左右切换展览"><div className="flower-carousel__slides">{files.map((file, index) => { let offset = index - active; if (offset > files.length / 2) offset -= files.length; if (offset < -files.length / 2) offset += files.length; return <figure key={file} className={offset === 0 ? 'is-active' : offset < 0 ? 'is-left' : 'is-right'}><img src={`/assets/virtual/${file}`} alt={`花，虚拟摄影组图 ${index + 1}`} /></figure> })}</div><button className="flower-carousel__control flower-carousel__control--prev" onClick={() => move(-1)} aria-label="上一张">←</button><button className="flower-carousel__control flower-carousel__control--next" onClick={() => move(1)} aria-label="下一张">→</button><div className="flower-carousel__meta"><span>FLOWERS / {String(active + 1).padStart(2, '0')}</span><b>2025.11.13</b></div></section> }
