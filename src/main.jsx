...
const projects = [
  ...
  { id:'shokz', no: '04', title: 'SHOKZ IN JAPAN', type: 'Cross-cultural marketing research', className: 'project--paper', note: '研究韶音进入日本市场时的文化适配、线下渠道与品牌认知，并提出本土化传播方案。', role: '跨文化研究 / 策略表达', evidence: '[补充本人具体分工]', time:'[补充项目时间]', learned:'将跨文化观察、消费者洞察与传播策略连接到具体市场情境。', outcome:'形成有关本土化营销、O2O 渠道与情感化品牌传播的方案。' },
  { id:'sony', no: '05', title: 'SONY / PERSONAL SOUND BUBBLE', type: 'Product visual presentation', className: 'project--green', note: '围绕 Sony WF-1000XM4 的场景、功能、竞争优势与目标用户建立产品叙事。', role: '产品研究 / 视觉演示', evidence: '[上传演示文稿]', time:'[补充项目时间]', learned:'练习将产品功能、使用场景和目标用户转译为有说服力的视觉展示。', outcome:'完成 Sony WF-1000XM4 产品视觉演示；请上传演示文稿。' },
  { id:'fellowship', no: '06', title: 'ROOTED FUTURES', type: 'Research fellowship / Cross-cultural education', className: 'project--fellowship', note: '将乡村振兴、人工智能时代的传播变革与跨国公益教育，收束为一组持续发生的社会观察与协作实践。', role: '项目参与 / 研究写作 / 课程制作 / 线上教学', evidence: '优秀营员 · 项目证书 · NGO Certificate · 课程论文（未发表）', time:'2025—2026', learned:'在社会议题、研究方法和实际协作之间建立连接，让抽象的文化与传播问题落实为可交付的内容。', outcome:'完成多个研究与教育协作项目，并保留证书、课程材料与论文作为过程证据。' },
]
...
function CasePage({ project }) {
  ...
  return <main className="detail-page"><header><a className="brand" href="#top">J.Z</a><PageBack /></header><section className={`case-cover ${project.className}`}><span>{project.no}</span><h1>{project.title}</h1><p>{project.type}</p></section>{project.url ? <BilibiliEmbed project={project} /> : <section className="case-video">{project.id === 'gbaa' ? <a href="/assets/gbaa/cultural-video.mp4" target="_blank" rel="noreferrer">WATCH PROJECT VIDEO <Arrow /></a> : <div>PROJECT MATERIAL<br /><small>[ 上传视频、演示稿或项目封面 ]</small></div>}</section>}{material}<section className="case-detail"><div><b>PROJECT OVERVIEW</b><p>{project.note}</p></div><div><b>WHEN / CONTEXT</b><p>{project.time}</p></div><div><b>WHAT I DID</b><p>{project.role}</p></div><div><b>WHAT I LEARNED</b><p>{project.learned}</p></div><div><b>OUTCOME / EVIDENCE</b><p>{project.outcome}<br /><br />{project.evidence}</p></div></section></main>
}
